...
angular.module('ngSuggest').run([
  ...
  function ($templateCache) {
    ...
    $templateCache.put('template/suggest-typeahead.html', '<a tabindex="-1" class="suggest-typeahead-item"><div bind-html-unsafe="match.label | htmlescape |  typeaheadHighlight:query" class="suggest-typeahead-label"></div><span ng-if="match.model.notation" style="color:#AAA;"> ({{match.model.notation}})</span><i ng-if="match.model.description" class="suggest-typeahead-description">{{match.model.description}}</i></a>');
  }
]);