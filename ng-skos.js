...
angular.module('ngSKOS').run([
  ...
  function ($templateCache) {
    ...
    $templateCache.put('template/skos-concept-mapping.html', '<div><div ng-if="!from.length && !to.length" style="margin-top:50px;margin-bottom:40px;text-align:center"><em>Please choose a concept scheme for mapping</em></div><div ng-if="from.length && to.length && !mapping.from.length && !mapping.to.length" style="margin-top:10px;margin-bottom:5px;text-align:center"><em>Please search for terms in the current concept schemes</em></div><div ng-if="mapping.from.length && !from.length && !to.length" style="margin-top:10px;margin-bottom:5px;text-align:center"><em>Please choose a term to work on</em></div><div ng-if="from.length || mapping.to[0].notation.length" style="width:100%"><table style="width:95%;margin:0 auto;align:center"><thead class="simple-list" style="text-align:center"><tr><th style="width:400px"><span ng-if="from.length" class="classification">{{from}}</span></th><th style="width:24px"></th><th style="width:400px;text-align:right"><span ng-if="to.length" class="classification">{{to}}</span></th></tr></thead><tbody><tr><td><div class="tmpl-border mappingResults-from"><div ng-if="mapping.from.length" skos-concept-list="mapping.from" on-select="selectFrom"></div><div ng-if="!mapping.from.length"><em>Please select a source mapping term</em></div></div></td><td><div style="margin:30px auto"><big><span class="glyphicon glyphicon-arrow-right"></span></big></div></td><td><div class="tmpl-border mappingResults-to" style="float:right"><div ng-if="mapping.to.length" skos-concept-list="mapping.to" on-select="selectTo"></div><div ng-if="!mapping.to.length"><em>Please select a target mapping term</em></div></div></td></tr></tbody></table></div><div ng-if="from.length && to.length"><div style="width:95%;margin:0 auto"><div style="border:1px solid #EEE;float:left;margin-bottom:5px;padding:4px"><div><span style="margin-left:12px">Choose mapping strength:</span></div><form style="margin-right:10px;display:inline"><input style="margin: 0px 12px" type="radio" name="strengthtype" ng-model="mapping.type.value" value="0">none<input style="margin: 0px 12px" type="radio" name="strengthtype" ng-model="mapping.type.value" value="3">strong<input style="margin: 0px 12px" type="radio" name="strengthtype" ng-model="mapping.type.value" value="2">medium<input style="margin: 0px 12px" type="radio" name="strengthtype" ng-model="mapping.type.value" value="1">weak</form></div><div style="float:right;margin-top:10px"><button ng-click="saveMapping()" ng-disabled="!mapping.from.length || !mapping.to.length">Save current mapping</button></div></div></div></div>');
    ...
    $templateCache.put('template/skos-concept.html', '<div class="skos-concept"><div class="top top-alt"><span ng-if="notation.length" class="notation">{{notation[0]}}</span> <b><span ng-if="prefLabel" skos-label="concept" lang="{{language}}"></span></b><a ng-if="uri && uri != notation" class="uri" href="{{uri}}" target="_blank"><span class="glyphicon glyphicon-globe"></span></a></div><div class="skos-concept-connected"><div class="skos-concept-altlabel"><ul><li ng-repeat="alt in altLabel"><span ng-if="$index < 5" style="display:inline">{{alt}}</span> <span style="margin-left:-4px;margin-right:3px" ng-if="$index < 4 && $index < altLabel.length-1">,</span></li></ul></div><div ng-if="broader.length" class="skos-concept-relation"><ul ng-repeat="c in broader"><li>&#8598; <span skos-label="c" lang="{{language}}" ng-click="click(c)"></span></li></ul></div><div ng-if="narrower.length" class="skos-concept-relation"><ul ng-repeat="c in narrower"><li>&#8600; <span skos-label="c" lang="{{language}}" ng-click="click(c)"></span></li></ul></div><div ng-if="related.length" class="skos-concept-relation"><ul ng-repeat="c in related"><li>&#10137; <span skos-label="c" lang="{{language}}" ng-click="click(c)"></span></li></ul></div><div ng-if="note">Note: <em>{{note.en[0]}}</em></div></div></div>');
    $templateCache.put('template/skos-mapping-collection.html', '<div ng-repeat="s in mappings"><div ng-repeat="(skey, source) in s"><div style="margin-bottom:6px"><span><big><b>{{skey | uppercase}}</b></big></span> <span style="margin-left:10px"><button style="line-height:90%" ng-if="hidden[skey] == false" ng-click="hide(skey)">Hide</button> <button style="line-height:90%" ng-if="hidden[skey] == true" ng-click="show(skey)">Show</button></span></div><div ng-if="hidden[skey] == false" ng-repeat="f in source"><span ng-repeat="(fkey, from) in f"><span ng-repeat="t in from"><span ng-repeat="(tkey, to) in t"><div skos-mapping-table="to" select-mapping="useMapping"></div></span></span></span></div></div><div ng-if="$index < mappings.length-1" style="margin-bottom:6px;border-bottom:1px solid #CCC"></div></div>');
    $templateCache.put('template/skos-mapping-table.html', '<table style="white-space:nowrap" class="table table-hover table-striped table-condensed table-bordered"><thead><tr><th><span class="classification">{{mapping[0].from[0].inScheme.notation[0]}}</span></th><th><span class="classification">{{mapping[0].to[0].inScheme.notation[0]}}</span></th><th><span>Type</span><a href="" style="text-decoration:none" ng-click="predicate = \'type.value\';reverse = !reverse"><small><span class="glyphicon glyphicon-sort"></span></small></a></th><th><span>Date</span><a href="" style="text-decoration:none" ng-click="predicate = \'timestamp\';reverse = !reverse"><small><span class="glyphicon glyphicon-sort"></span></small></a></th><th>Actions</th></tr></thead><tbody><tr ng-repeat="m in mapping | orderBy:predicate:reverse"><td><ul class="simple-list" style="margin-bottom:0px"><li ng-repeat="f in m.from"><span class="notation" popover="{{f.prefLabel.de}}" popover-trigger="mouseenter">{{f.notation[0]}}</span></li></ul></td><td><ul class="simple-list" style="margin-bottom:0px"><li ng-repeat="t in m.to"><span class="notation" popover="{{t.prefLabel.de}}" popover-trigger="mouseenter">{{t.notation[0]}}</span></li></ul></td><td>{{m.type.prefLabel.en}} ({{m.type.value}})</td><td>{{m.timestamp}}</td><td style="text-align:center"><a ng-if="select" style="cursor:pointer" ng-click="select(m)"><span class="glyphicon glyphicon-check"></span></a></td></tr></tbody></table>');
